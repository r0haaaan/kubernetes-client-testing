...
public class EclipseJKubeDeploymentPipeline {
    ...
    public static final String JKUBE_DEPLOYMENT_PIPELINE_RUN = "jkube-pipeline-run";
    ...
    public static void main(String[] args) {
        try (TektonClient tkn = new DefaultTektonClient()) {
            ...
            //tektonSetup(tkn, namespace);
            tektonCleanup(tkn, namespace);
        }
    }

    private static void tektonSetup(TektonClient tkn, String namespace) {
    ...
    private static void tektonCleanup(TektonClient client, String namespace) {
        logger.info("Deleting PipelineRun");
        client.v1beta1().pipelineRuns().inNamespace(namespace).withName(JKUBE_DEPLOYMENT_PIPELINE_RUN).delete();
        client.v1beta1().pipelineRuns().inNamespace(namespace).withName("tutorial-pipelinerun").delete();
        logger.info("Deleting Task");
        client.v1beta1().tasks().inNamespace(namespace).withName("deploy-task").delete();
        logger.info("Deleting Pipeline");
        client.v1beta1().pipelines().inNamespace(namespace).withName("jkube-pipeline").delete();
        logger.info("Deleting PipelineResource");
        client.v1alpha1().pipelineResources().inNamespace(namespace).withName("jkube-demo-git").delete();
    }
}
